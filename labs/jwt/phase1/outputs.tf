...
# Used by Phase 3 for ECS task networking (awsvpcConfiguration)
output "vpc_id" {
...
# Used by Phase 3 for Service_B ECS service networking (allows callers inbound on 5000)
output "service_b_security_group_id" {
  description = "Security group ID for Service_B (inbound from callers on port 5000)"
...
# Used by Phase 3 in Service_A task definition
output "service_a_task_role_arn" {
  description = "ARN of Service_A task role (structural parity - no special permissions)"
  value       = aws_iam_role.service_a_task.arn
}

# Used by Phase 3 in Service_B task definition
output "service_b_task_role_arn" {
  description = "ARN of Service_B task role (structural parity - no special permissions)"
  value       = aws_iam_role.service_b_task.arn
}

# Used by Phase 3 in Service_C task definition
output "service_c_task_role_arn" {
  description = "ARN of Service_C task role (structural parity - no special permissions)"
